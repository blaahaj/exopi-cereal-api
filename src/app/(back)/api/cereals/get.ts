...
export const GET = async (req: NextRequest) => {
  ...
  let queryBuilder = AppDataSource.getRepository(DBCereal).createQueryBuilder();
  ...
  return await queryBuilder.getMany().then((cereals) => {
    const data: { cereals: TCerealWithID[] } = { cereals };
