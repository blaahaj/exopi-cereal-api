...
export default function Home() {
  ...
  const columns: GridColDef<CerealWithID>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        renderCell: (t) => (
          <span title={`${t.row.calories} kCal per serving`}>
            {t.row.calories}
          </span>
        ),
      },
      ...
      },
      {
        ...
        renderCell: (t) => (
          <span title={`${t.row.fat}g fat`}>{t.row.fat}g</span>
        ),
      },
      {
        field: "sodium",
  ...
  const initialVisibility = useMemo(() => {
    const r: Record<(typeof columns)[number]["field"], boolean> = {};
